...
if (export_plots_to_plots_folder) {
  plotname=file.path(plot_dir,"S5a_SelectedCoxModelVariables.pdf") #Comment to SIP 
  pdf(file=plotname, height =8, width = 8, paper = "special") #Comment to SIP
}
...
mtext("Selected Cox Model Variables",side=3,outer = TRUE,line=3)
if (export_plots_to_plots_folder) {
  dev.off()
}